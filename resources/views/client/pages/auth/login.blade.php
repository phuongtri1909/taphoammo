...
@section('header')
    <h2 class="text-2xl md:text-xl sm:text-lg font-semibold text-[#002740] mb-2">Chào mừng quay trở lại</h2>
    <p class="text-sm sm:text-xs text-[#7E899C]">Đăng nhập để tiếp tục</p>
@endsection
...
@section('content')
    ...
    <form method="POST" action="{{ route('sign-in.post') }}" class="login-form">
        @csrf

        <div class="mb-6">
            <label for="email" class="block text-sm font-normal text-[#002740] mb-2">Email</label>
            <input type="email" class="w-full py-3.5 px-4 rounded-full border border-[#E7ECF1] bg-[#F8F9FB] text-sm text-primary-8 transition-all login-input @error('email') border-red-500 @enderror" id="email" name="email" value="{{ old('email') }}" placeholder="Nhập email" required autofocus>
            @error('email')
        ...
        <div class="mb-6">
            <label for="password" class="block text-sm font-normal text-[#002740] mb-2">Mật khẩu</label>
            <div class="password-input-wrapper">
                <input type="password" class="w-full py-3.5 px-4 rounded-full border border-[#E7ECF1] bg-[#F8F9FB] text-sm text-primary-8 transition-all login-input @error('password') border-red-500 @enderror" id="password" name="password" placeholder="Nhập mật khẩu" required>
        ...
        <div class="flex justify-between items-center mb-6">
            <div class="flex items-center">
                <input type="checkbox" class="w-[18px] h-[18px] border-2 border-gray-300 rounded cursor-pointer checked:bg-primary checked:border-primary" id="remember" name="remember" {{ old('remember') ? 'checked' : '' }}>
                <label class="text-sm font-normal text-[#7E899C] ml-2 cursor-pointer" for="remember">Lưu thông tin đăng nhập</label>
            </div>
            <a href="{{ route('forgot-password') }}" class="text-sm text-[#E82A00] underline font-medium transition-opacity forgot-password-link">Quên mật khẩu?</a>
        </div>
    ...
    <div class="mt-6 text-center">
        <p class="text-sm font-normal text-[#7E899C] m-0">
            ...
            <a href="{{ route('sign-up') }}" class="text-sm text-[#E82A00] underline font-medium transition-opacity register-link">Đăng ký ngay.</a>
        </p>
    </div>
@endsection
